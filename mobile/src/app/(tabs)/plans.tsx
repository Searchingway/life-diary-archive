...
function extraOf(record: NewRecord): PlanExtra {
  return {
    ...
    startDate: String(record.extra.startDate || record.date || today()),
    deadline: String(record.extra.deadline || ""),
    priority: (PLAN_PRIORITIES.includes(record.extra.priority as PlanPriority) ? record.extra.priority : "中") as PlanPriority,
    tasks: Array.isArray(record.extra.tasks) ? (record.extra.tasks as PlanTask[]) : [],
  };
}

function newTask(): PlanTask {
  return { id: `task-${Date.now()}-${Math.random().toString(16).slice(2)}`, title: "", done: false };
}
...
export default function PlansScreen() {
  ...
  function updateExtra(next: PlanExtra) {
    if (draft) setDraft({ ...draft, extra: next });
  }
    ...
    return (
      <Screen action={<SecondaryButton label="返回列表" onPress={() => setDraft(null)} />} subtitle="保存失败时会保留当前填写内容" title={draft.id ? "编辑计划" : "新建计划"}>
        ...
        <Panel>
          ...
          <Field label="开始日期" onChangeText={(startDate) => updateExtra({ ...extra, startDate })} value={extra.startDate} />
          <Field label="截止日期" onChangeText={(deadline) => updateExtra({ ...extra, deadline })} value={extra.deadline} />
          ...
          <Field label="备注" multiline onChangeText={(body) => setDraft({ ...draft, body })} value={draft.body} />
        </Panel>
        ...
        {extra.tasks.map((task) => (
          <Panel key={task.id}>
            ...
            <Field label="任务日期（可选）" onChangeText={(date) => updateExtra({ ...extra, tasks: extra.tasks.map((item) => (item.id === task.id ? { ...item, date } : item)) })} value={task.date || ""} />
    ...
    <Screen action={<PrimaryButton label="新建计划" onPress={() => setDraft(emptyPlan())} />} subtitle="按逾期、今日任务、状态和截止日期排序" title="计划">
      ...
      <Panel>
        {!plans.length ? <EmptyState text={loading ? "正在读取…" : "还没有计划，先新建一个吧。"} /> : plans.map((plan) => {
          ...
          const overdueDays = plan.extra.deadline && plan.extra.deadline < today() && plan.status !== "已完成" && plan.status !== "已暂停"
            ? Math.floor((Date.parse(`${today()}T00:00:00`) - Date.parse(`${plan.extra.deadline}T00:00:00`)) / 86_400_000)
            : 0;
          return (
            <Pressable key={plan.id} onPress={() => setDraft(editable(plan))} style={styles.planRow}>
              ...
              <Text style={[styles.planMeta, overdueDays > 0 && styles.overdue]}>{overdueDays > 0 ? `已逾期 ${overdueDays} 天` : plan.extra.deadline ? `截止：${plan.extra.deadline}` : "未设置截止日期"}</Text>
            </Pressable>
          );
        })}
      </Panel>

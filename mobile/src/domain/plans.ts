...
export function emptyPlan(): NewRecord {
  return {
    ...
    status: "未开始",
    type: "add",
    extra: { schema_version: 2, goal: "", start_date: today(), due_date: "", priority: "中", notes: "", tags: [], tasks: [], plan_type: "add" },
  };
}
...
function deadlineSortValue(plan: PlanMemo): string {
  return plan.extra.due_date || "9999-12-31";
}

function isOverdue(plan: PlanMemo, selectedDate: string): boolean {
  return Boolean(plan.extra.due_date && plan.extra.due_date < selectedDate && plan.status !== "已完成" && plan.status !== "已暂停");
}

function hasOpenTaskOn(plan: PlanMemo, selectedDate: string): boolean {
  return plan.extra.tasks.some((task) => task.scheduled_date === selectedDate && !task.done);
}
...
export function selectTodayPlanTasks(plans: PlanMemo[], selectedDate = today()): TodayPlanTask[] {
  return plans.flatMap((plan) =>
    plan.extra.tasks
      .filter((task) => task.scheduled_date === selectedDate && !task.done)

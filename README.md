# 人生档案 Diary - Desktop 3.2 信息备忘模块

当前桌面端为 Desktop 3.2，新增「信息备忘」模块。核心分工是：手机版负责每天快速记录，电脑版负责导入、整理、分析、复盘和归档。所有数据仍然本地优先，保存到 `data/Diary/`，新增模块继续兼容原有目录结构，不做云同步、账号登录或 AI 自动分析。
...
- 总览页显示基础统计和最近记录时间线
- 足迹模块完整 CRUD
- 旧读书笔记数据兼容到“作品感悟”，默认作品类型为“书籍”
- 教训与反思模块完整 CRUD
- 自我分析模块完整 CRUD
- 作品感悟模块完整 CRUD
- 日记支持按日期搜索和查看历史记录
- 足迹支持按地点 / 简介 / 日期 / 感悟搜索
- 轻思考模块完整 CRUD，支持追加想法、状态整理和转成轻计划
- 轻资源模块完整 CRUD，支持时间 / 金钱 / 精力 / 情绪等资源项、轮回测试、转轻计划和复制为教训反思
- 自我观察模块支持导入查看、筛选和转成自我分析草稿
- 信息备忘模块完整 CRUD，支持三种类型（接单记录、网课资源、通用信息），含类型专属字段、搜索和筛选
- 教训与反思支持按标题 / 类型 / 标签 / 正文搜索，并可按事件类型筛选
- 自我分析支持按标题 / 分析类型 / 标签 / 正文搜索，并可按分析类型筛选
- 作品感悟支持按标题 / 作品类型 / 状态 / 标签 / 正文搜索，并可按作品类型筛选
- 日记 / 足迹 / 旧读书笔记 / 作品感悟都支持图片兼容读取
- 教训与反思支持添加截图 / 图片、打开图片、移除图片和图片备注名
- 自我分析支持添加图片 / 截图、打开图片、移除图片和图片备注名
- 作品感悟支持添加图片 / 海报、打开图片、移除图片和图片备注名
- 足迹和当日日记支持双向跳转
- 旧读书笔记仍支持通过后台兼容逻辑读取；桌面入口统一为“作品感悟”
- 教训与反思支持关联若干篇日记，并从反思页直接打开关联日记
- 自我分析支持关联已有日记和教训与反思，并可从自我分析页打开关联日记或教训
- 作品感悟支持关联已有日记和自我分析，并可从作品页打开关联日记或自我分析
- 足迹采用“两层结构”：地点档案 + 多个日期关联记录
- 轻计划支持本地 CRUD、搜索、筛选、标记完成，并支持“加法计划 / 减法计划”
- 总览页升级为全模块时间线，接入日记、足迹、作品感悟、轻计划、教训反思、自我分析、轻思考、轻资源、自我观察、信息备忘
- 数据管理中心支持备份、恢复、导入手机版 ZIP、导出电脑版 ZIP、查看数据目录、打开数据文件夹、模块数量统计和健康检查
- 手机版 `LifeDiary_Backup_yyyyMMdd_HHmmss.zip` 可直接导入电脑版；导入前自动备份当前 `data/Diary/`，导入采用按 id 合并规则
- 日记支持按时间范围导出多天内容为 Word 和 PDF
- PDF 由 Word 文档直接转换生成，保持一致版式
- 日记 / 足迹 / 旧读书三模块仍保留压缩包交换包能力
- 日记页包含最近 18 周绿色热力统计图
...
Desktop 3.2 新增「信息备忘」模块，用于记录不适合写进日记但容易遗忘的碎片化信息。

- 新增桌面端「信息备忘」页签，数据保存到 `data/Diary/info_memos/`，每条记录为 `info_memo.json`，采用单 JSON 文件存储。
  - 三种信息类型：**接单记录**（客户、中介、执行人、日期、工期、报价、定金、尾款、交付内容）、**网课资源**（课程名称、平台、链接、学习方向、购买状态、进度、原因）、**通用信息**（分类、主要内容、提醒日期）。
  - 每种类型有独立的状态列表，不混用。
  - 通用字段：标题、类型、状态、优先级、标签、来源、相关链接、本地路径、备注。
- 支持新建、编辑、保存、删除、搜索、按类型筛选、按状态筛选。
- 类型筛选和状态筛选联动。
- 接入自动保存，停止编辑约 3 秒后保存有内容的记录，自动保存不会导致光标跳到开头。
- 总览页时间线和模块统计已接入信息备忘。
- 顶部导航「信息备忘」放在「轻资源」后、「自我观察」前。

## Desktop 3.1 工程结构整理版

Desktop 3.1 的目标是补全日志系统、统一版本号、修正文档和代码的不一致，为后续功能开发打好工程基础。

- 新增“导入手机版 ZIP”能力，可选择 `LifeDiary_Backup_yyyyMMdd_HHmmss.zip`，读取 `manifest.json` 中的来源平台、备份时间和模块列表。
- 导入前自动生成 `BeforeMobileImportBackup_*.zip`，导入失败会尽量恢复导入前数据；导入方式是合并而不是覆盖，同 id 数据按 `updated_at` / `created_at` 判断是否更新，不同 id 追加导入。
- 新增桌面端“轻思考”页签，数据保存到 `data/Diary/thoughts/`，每条记录为 `thought.json`，支持列表、新建、编辑、保存、删除确认、搜索、类型筛选、状态筛选、追加想法和转轻计划。
- 新增桌面端“轻资源”页签，数据保存到 `data/Diary/resources/`，每条记录为 `resource.json`，支持时间、金钱、精力、情绪、勇气、身体、注意力、风险、机会成本等资源项，以及轮回测试。
- 新增桌面端“自我观察”页签，数据保存到 `data/Diary/observations/`，每条记录为 `observation.json`，支持按情绪、强度、日期、触发原因筛选，并可转为自我分析草稿。
- 总览页升级为全模块时间线，最近记录覆盖日记、足迹、作品感悟、轻计划、教训反思、自我分析、轻思考、轻资源、自我观察，并显示模块数量和最近更新时间。
- 新增“数据管理中心”页签，集中提供备份整个 `data/Diary/`、恢复备份、导入手机版 ZIP、导出电脑版 ZIP、查看目录、打开数据文件夹、模块数量统计和数据健康检查。
- 原“读书笔记”桌面入口统一升级为“作品感悟”。旧数据继续保存在 `data/Diary/books/`，电脑版会作为作品类型“书籍”兼容显示；新作品感悟默认保存到 `data/Diary/works/`。
- 作品类型支持：书籍、电影、动漫、游戏、文章、视频、课程、其他。

手机版 ZIP 导入合并规则：

- ZIP 根目录需要有 `manifest.json`，数据目录需要是 `Diary/`。
- 支持识别 `entries/`、`footprints/`、`books/`、`plans/`、`lessons/`、`self_analysis/`、`works/`、`thoughts/`、`resources/`、`observations/`。
- 当前电脑端数据不会被整包替换；导入时逐条记录按 id 合并。
- 目标端没有的 id 会直接导入；目标端已有相同 id 时，只有来源记录更新时间更晚才更新，否则跳过。
- 图片随对应记录目录一起导入或更新，导入结果会统计新增、更新、跳过和图片数量。
...
2.7 版本新增桌面端“作品感悟”页签，用于记录电影、动画、游戏、剧集、漫画、音乐、播客、纪录片等作品带来的感受、思考和自我联想：

- 新增桌面端“作品感悟”页签，不联网抓取资料，不自动下载封面，不加入 AI 自动影评或推荐系统。
- 作品感悟数据保存到 `data/Diary/works/`，每条记录包含 `work.json`、`content.md` 和 `images/`。
- 支持新建、查看、编辑、保存、恢复已保存内容、软删除、搜索和按作品类型筛选。
- 支持设置作品类型、创作者、状态、开始日期、完成日期、评分和标签。
- 正文按一句话印象、内容摘要、喜欢的地方、不喜欢的地方、触动我的地方、喜欢的角色 / 场景 / 台词、想到的自己、过去经历关联和最终评价分段保存。
- 支持关联已有日记、打开关联日记、关联已有自我分析，并可从作品页打开关联自我分析。
- 支持添加图片 / 海报、打开已添加图片、移除图片和填写图片备注名。
- 作品感悟页接入桌面端自动保存；停止编辑约 3 秒后会尝试保存有内容的记录。
- 备份恢复识别 `works/` 模块；恢复完成后会刷新作品感悟页面。
- 本版本只更新桌面端功能，未修改 Android 目录，未构建 APK。
## 界面预览
...
- `entries/` 保存日记
- `footprints/` 保存地点足迹
- `books/` 保存旧读书笔记，Desktop 3.0 会作为“作品感悟 / 书籍”兼容读取
- `works/` 保存新作品感悟
- `thoughts/` 保存轻思考
- `resources/` 保存轻资源
- `observations/` 保存自我观察
- `info_memos/` 保存信息备忘，每条记录为 `info_memo.json`，按类型存储不同字段
...
- `books/<book_id>/notes.md` 保存旧读书笔记正文；新作品感悟正文保存到 `works/<work_id>/content.md`
...
- 主窗口十二页签：`总览` / `日记` / `足迹` / `轻计划` / `轻思考` / `轻资源` / `信息备忘` / `自我观察` / `教训与反思` / `自我分析` / `作品感悟` / `数据管理`
- 总览统计区、全模块时间线、日记列表、地点足迹列表、作品感悟列表、轻计划列表、反思记录列表、轻思考列表、轻资源列表、自我观察列表
- 新建、查看、编辑、保存、删除
...
- 地点档案下可新增多个日期关联
- 每个日期关联可单独写感悟和上传图片
- 旧读书笔记通过“作品感悟”入口按“书籍”类型显示，原 `books/` 数据不迁移、不删除
...
- 作品感悟可新建、保存、搜索、按作品类型筛选、软删除、关联日记、关联自我分析和管理图片
- 总览页可刷新并展示 8 个基础统计数字、全模块数量和最近 30 条全模块时间线（含信息备忘）
- 总览页可手动备份数据，也可从合法 zip 备份包恢复数据；数据管理页提供手机版 ZIP 导入、电脑版 ZIP 导出和健康检查
- 日记、足迹、轻计划、教训反思、自我分析、作品感悟、轻思考、轻资源、自我观察、信息备忘等可编辑页支持自动保存，状态栏会提示已保存、有未保存修改、自动保存中、已自动保存或自动保存失败
- 日记页显示最近 18 周热力统计图
...
- 主窗口上方是十二个页签：`总览`、`日记`、`足迹`、`轻计划`、`轻思考`、`轻资源`、`信息备忘`、`自我观察`、`教训与反思`、`自我分析`、`作品感悟`、`数据管理`
- 总览页顶部是本月 / 今年的日记和计划统计，中间显示全模块数量和最近更新时间，下方是最近 30 条全模块时间线（含信息备忘）
- 总览页右上角保留“备份数据”和“恢复备份”按钮；导入手机版 ZIP 等完整操作在“数据管理”页签中
...
- 作品感悟页左侧是作品列表，支持按作品类型、状态、标签和关键词筛选
- 作品感悟页右侧用于编辑作品名称、作品类型、创作者、状态、标签、摘录 / 片段、理解、感受和关联记录
- 旧 `books/` 读书笔记会在作品感悟列表中以“书籍”类型显示；保存为新作品感悟时写入 `works/`，旧数据仍保留
...
- 作品感悟页用于记录电影、动画、游戏、剧集、漫画、音乐、播客、纪录片等作品带来的感受和自我联想，不联网抓取资料、不自动影评、不做推荐系统
- `保存` 用于提交当前页的文字和图片改动
...
- 旧读书笔记可作为“书籍”类型作品感悟读取，旧正文来自 `summary.md` 和 `notes.md`
- 新作品感悟可记录书籍、电影、纪录片、动漫、游戏、文章、音乐、课程、视频、展览和其他作品
- 可填写创作者、阅读或观看状态、开始时间、完成时间、标签、评分或重要程度
- 可写摘录 / 片段、我的理解、我的感受、备注
- 可关联日记、自我分析和轻计划线索
- 关联日记或自我分析后可以直接从作品感悟页打开对应记录
...
- 支持添加图片 / 截图、打开图片、移除图片和填写图片备注名。
## 作品感悟模块
- 可记录电影、动画、游戏、剧集、漫画、音乐、播客、纪录片和其他作品类型。
- 可设置想看、进行中、已完成、暂停、放弃五种状态，并记录创作者、开始日期、完成日期、评分和标签。
- 正文按一句话印象、内容摘要、喜欢的地方、不喜欢的地方、触动我的地方、喜欢的角色 / 场景 / 台词、想到的自己、过去经历关联和最终评价组织。
- 支持按标题、作品类型、状态、标签、正文、关联记录和图片备注搜索。
- 支持关联已有日记并从作品页打开，也支持保存和打开关联自我分析。
- 支持添加图片 / 海报、打开图片、移除图片和填写图片备注名。
- 不联网抓取豆瓣、Bangumi、Steam、IMDb 等数据，不自动下载封面，不新增 AI 自动影评或推荐系统。
## 总览页
...
- 时间线已升级为全模块时间线，接入日记、足迹、作品感悟、轻计划、教训反思、自我分析、轻思考、轻资源和自我观察。
...
- 非法备份包不会覆盖当前数据。
- “导入手机版 ZIP”会读取 `manifest.json`，导入前自动备份当前电脑端 `data/Diary/`，并按 id 合并数据。
- “导出电脑版 ZIP”会生成当前电脑端完整备份包。
- 数据健康检查会检查 JSON 是否能解析、关键目录是否存在、图片目录和 manifest 情况，并用普通中文给出结果。
- 恢复或导入完成后会刷新总览、日记、足迹、作品感悟、轻计划、教训反思、自我分析、轻思考、轻资源、自我观察和数据管理页面。
...
- 日记、足迹、旧读书笔记和作品感悟都采用“每条记录一个独立目录”的本地存储方式
...
- 作品感悟采用独立目录保存，每条作品感悟对应一个 `work.json`、一个 `content.md` 和一个 `images/` 目录
- 轻思考采用独立目录保存，每条记录对应一个 `thought.json`
- 轻资源采用独立目录保存，每条记录对应一个 `resource.json`
- 自我观察采用独立目录保存，每条记录对应一个 `observation.json`
- 信息备忘采用独立目录保存，每条记录对应一个 `info_memo.json`，类型专属字段存入 `type_fields` 字典
...
- 自我分析存储测试
- 作品感悟存储测试
- 总览页聚合测试，覆盖基础统计和全模块时间线记录
- 自动保存测试，覆盖日记、轻计划、教训与反思、自我分析、作品感悟、页签切换、图片备注、恢复已保存内容和空白草稿
- 备份恢复与手机版导入测试，覆盖备份包结构、manifest、合法/非法校验、安全备份、合并导入和失败不覆盖
- 日记导出测试
- 压缩包交换包导出测试
- 主窗口多页签与跨页跳转烟雾测试
...
- 桌面端新增「信息备忘」页签，支持三种信息类型：接单记录、网课资源、通用信息。
- 新增 `data/Diary/info_memos/` 数据目录，每条记录保存为 `info_memo.json`。
- 支持信息卡片的新建、编辑、保存、删除、搜索、按类型筛选、按状态筛选。
- 类型筛选和状态筛选联动；切换类型时自动更新可用状态。
- 信息备忘页接入自动保存；自动保存不会导致光标跳到开头。
- 总览页时间线和模块统计已接入信息备忘。
- 顶部导航新增「信息备忘」页签，放置在「轻资源」后、「自我观察」前。
- 本版本只更新桌面端功能，Android 手机版暂不变。
### 2.7

- 桌面端新增“作品感悟”页签，支持记录电影、动画、游戏、剧集、漫画、音乐、播客、纪录片和其他作品感悟。
- 新增 `data/Diary/works/` 数据目录，每条记录保存为 `work.json + content.md + images/`。
- 支持作品感悟记录的新建、编辑、保存、恢复、软删除、搜索、类型筛选、状态显示、图片管理、关联日记和关联自我分析。
- 作品感悟页接入自动保存；备份恢复识别 `works/` 模块。
- 本模块不联网抓取资料、不自动下载封面、不加入 AI 自动影评或推荐系统。
- 本版本只更新桌面端功能，Android 手机版暂不变。
### 2.6

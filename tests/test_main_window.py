...
from life_dairy.models import LessonRelatedDiary
...
class DiaryMainWindowTests(unittest.TestCase):
    ...
    def test_main_window_has_desktop_tabs(self) -> None:
        self.assertEqual(12, self.window.tabs.count())
        ...
        self.assertEqual("信息备忘", self.window.tabs.tabText(6))
        self.assertEqual("自我观察", self.window.tabs.tabText(7))
        self.assertEqual("教训与反思", self.window.tabs.tabText(8))
        self.assertEqual("自我分析", self.window.tabs.tabText(9))
        self.assertEqual("作品感悟", self.window.tabs.tabText(10))
        self.assertEqual("数据管理", self.window.tabs.tabText(11))
    ...
    def test_tab_switching_does_not_leave_work_or_book_panel_overlaid(self) -> None:
        self.window.show()
        ...
        pages = [getattr(self.window, name) for name in page_names]

        for index, current_page in enumerate(pages):
            self.window.tabs.setCurrentIndex(index)
            self.app.processEvents()
            self.assertIs(self.window.tabs.currentWidget(), current_page)
            self.assertTrue(current_page.isVisible(), page_names[index])
            for other_index, other_page in enumerate(pages):
                if other_index != index:
                    self.assertFalse(other_page.isVisible(), f"{page_names[other_index]} visible over {page_names[index]}")
    ...
    def test_legacy_book_data_opens_from_work_page_as_book_type(self) -> None:
        book = self.book_storage.create_empty_book()
        book.title = "人类简史"
        book.author = "尤瓦尔"
        saved_book = self.book_storage.save_book(book)

        self.window.work_page.refresh_work_list(select_id=saved_book.id)
        self.window.work_page.open_work_by_id(saved_book.id)

        self.assertEqual(saved_book.id, self.window.work_page.current_work.id)
        self.assertEqual("书籍", self.window.work_page.current_work.work_type)
        self.assertEqual("人类简史", self.window.work_page.current_work.title)
    ...
    def test_self_analysis_page_can_open_related_diary_and_lesson(self) -> None:
        diary = self.diary_storage.create_empty_entry()
        diary.date = "2026-05-01"
        diary.title = "自我分析关联日记"
        saved_diary = self.diary_storage.save_entry(diary)
        ...
        self.window._open_diary_from_self_analysis(saved_diary.id, saved_diary.date)
        self.assertIs(self.window.tabs.currentWidget(), self.window.diary_page)
        self.assertEqual(saved_diary.id, self.window.diary_page.current_entry.id)
